...
class SectionFrame(wx.Frame):
    ...
    def __init__(self, croco=None, variableName=None, variable=None,
                 ...
                 sliceIndex=None, timeIndex=None):
...
class CrocoGui(wx.Frame):
    ...
    def animate(self):
        for i in range(self.startTimeIndex, self.endTimeIndex + 1):
            ...
            wx.CallAfter(self.drawxy, setlim=False, setcol=False, anim=True)
            if self.shouldAbort:
                break
        wx.CallAfter(self.animateDone, self.shouldAbort)
    ...
    def updateVariableXY(self, setlim=True):
        ...
        if depth > 0:
            ...
            if self.variableName in self.croco.ListOfVariables:
                ...
                try:
                    self.variableXY = self.croco.variables[self.variableName]\
                        .isel(t=self.timeIndex, z_r=self.levelIndex)
                except Exception:
                    # 3D variable w level
                    try:
                        self.variableXY = self.croco.variables[self.variableName]\
                            .isel(t=self.timeIndex, z_w=self.levelIndex)
                    # 2D variable
                    except Exception:
        ...
        # Depth plot
            ...
            # Derived variable
                ...
                if 'pv' in self.variableName:
                    var = get_pv(self.croco, self.timeIndex, depth=depth, minlev=minlev,
